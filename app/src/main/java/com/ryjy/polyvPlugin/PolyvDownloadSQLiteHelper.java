...
public class PolyvDownloadSQLiteHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(
                "create table if not exists downloadlist(vid varchar(20),title varchar(100),duration varchar(20),filesize int,bitrate int,percent int default 0,total int default 0,errorMsg varchar(512),primary key (vid, bitrate))");
    }
     ...
     * @param info
     */
    public void updateErrorMsg(PolyvDownloadInfo info) {
        SQLiteDatabase db = getWritableDatabase();
        String sql = "update downloadlist set errorMsg=? where vid=? and bitrate=?";
        db.execSQL(sql, new Object[]{info.getErrorMsg(), info.getVid(), info.getBitrate()});
    }
